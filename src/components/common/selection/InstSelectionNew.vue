<template>
  <div class="tree-leve" style="width: 100%" id="box">
    <input :placeholder="placeholder" class="inputClass" v-model="name" @focus="onFocus"/>
    <el-card v-show="treeVisible" :body-style="{ padding: '0px' } " style="width: 100%;">
      <el-tree
        ...
        lazy
        @node-click="handleNodeClick" style="padding:10px 0; width: 100%;">
...
<script>
  import { role } from '@/util/roleFunction.js'
  ...
  export default {
    ...
    created () {
      ...
      let instLevel = sysInstInfo.instLevel
    ...
    methods: {
      append (node, data) {
        ...
        instGetById(this, instId, Toast).then((res) => {
          let data = res.data
          if (!data) {
            Toast({
              message: res.msg,
      ...
      handleNodeClick (node, data) {
        ...
        this.name=node.aliasName;
        bus.$emit('instId', node.instId);
      ...
      instGetAllByIdFun (instId, resolve, node) {
        instGetAllById(this, instId, Toast).then((res) => {
            let data = res.data
            if (!data) {
              Toast({
                message: res.msg,
