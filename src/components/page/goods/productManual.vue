<template>
  <div>
    ...
    <div class="top-btn">
      ...
      <el-button size="small" type="danger" class="top-btn-right" @click="onTruncatTap" style="margin-left: 10px;">清空
      </el-button>
    </div>
    <div class="list-name">产品说明书展示</div>
        ...
        <transition-group tag="div" id="goodsDesc" class="item-ul">
          <div v-for="(item,index) in goodsDesc" :key="index" class="drag-list-desc">
            ...
            <div v-else class="drag-list-text-desc"
                 :class="index==0?'drag-list-top':(index==goodsDesc.length-1?'drag-list-bottom':'{}')">{{item.cont}}
            </div>
  ...
  export default {
    ...
    methods: {
      initData() {
        ...
        getGoodsProductmanualById(this, params).then(
          (res) => {
            if (res.data != null) {
              try{
                this.version=res.data.version;
                this.goodsDesc = JSON.parse(res.data.productManual || '[]');
              }catch (e) {
                this.goodsDesc=[];
              }
              this.oper=2;
      ...
      onSaveTap() {
        if (this.oper === 1) {
          ...
          let params={};
          params.goodsId = this.goodsId;
          params.productManual= JSON.stringify(this.goodsDesc);
          addGoodsProductmanual(this, params).then(
            res=>{
        ...
        if (this.oper === 2) {
          ...
          updateGoodsProductmanualById(this, params).then(
            res=>{
