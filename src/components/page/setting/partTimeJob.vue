    ...
    <el-table :data="tableData" border stripe class="el-table-plus">
        ...
        <template slot-scope="props">
          <ul class="table-ul">
            <li v-for="item in props.row.positionAll" :key="item">
              <el-button type="primary" @click="showPositionDetail(positionAll[item])">{{positionAll[item].postionName}}
              </el-button>
            </li>
          </ul>
        </template>
      ...
      <el-table-column label="操作" width="340" >
        <template slot-scope="props">
          ...
          <el-button v-if="props.row.departmentFlag==='2'" type="danger" @click="deleteTap(props.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteCommit(item){
        console.log('deleteCommit item', item);//debug
        let params={};
