...
var json2sha = function (jsonstr1) {

  let jsonstr2 = this.SHA256(jsonstr1);
  return jsonstr2;
};
...
var treeSort = function (tree) {
  ...
  tree.forEach(itemTree => {
    if (itemTree.menuBeans) {
      let menuBeans = [];
      functionDetail.Function.forEach(itemFun => {
        itemTree.menuBeans.forEach(itemMenu => {
          if (itemMenu.funcId === itemFun.funcId) {
            menuBeans.push(itemMenu);
          }
        });
      });
      itemTree.menuBeans = menuBeans;
    }
  });
  functionDetail.Function.forEach(itemFun => {
    tree.forEach(itemTree => {
      if (itemFun.funcId === itemTree.funcId) {
        newTree.push(itemTree);
      }
    });
  });
  return newTree;
