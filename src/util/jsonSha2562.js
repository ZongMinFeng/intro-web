define(function(require) {
	...
	util.getSignRValue = function() {
		return signRValue;
	};
	util.mathJsonSha = function(arys, key, flag) {
		var result = "";
		var arys1 = {};
		var arys2 = {};
	...
	util.json2sha = function(arys, arys1, flag) {
		...
		for (var i = 0; i < this.getSignRValue().R.length; i++) {
			var packR = [];
			if (jsonObj.Y051 !== this.getSignRValue().R[i].TxnId) {
				continue;
			}
			if (flag === 1 && this.getSignRValue().R[i].Req !== undefined) {
				packR = this.getSignRValue().R[i].Req;
			} else if (flag === 2 && this.getSignRValue().R[i].Res !== undefined) {
				packR = this.getSignRValue().R[i].Res;
			} else {
				break;
			}
	...
	util.jsonR2Value = function(arys1, arys2) {
		var jsonstr = "";
	...
	util.jsonR2Unpack = function (arys1) {
		var jsonstr = "";
